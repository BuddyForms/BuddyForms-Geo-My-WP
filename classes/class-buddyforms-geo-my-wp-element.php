...
class BuddyFormsGeoMyWpElement {
	...
	public function ajax_delete_field_row() {
		try {
			...
			if ( ! isset( $_POST['action'] ) || ! isset( $_POST['_nonce'] ) || ! $is_valid || ! isset( $_POST['post_id'] ) || ! isset( $_POST['field_number'] ) || ! isset( $_POST['field_name'] ) || ! isset( $_POST['form_slug'] ) ) {
				die( 1 );
			}
			...
			$remove_result = $this->delete_address_element( $id, $slug, $form_slug, $location_id );
	...
	public function delete_address_element( $related_id, $slug, $form_slug, $location_id ) {
		$result = false;
		if ( ! empty( $related_id ) ) {
			...
			if ( ! empty( $location_id ) ) {
				$del3 = GMW_Location::delete( self::get_buddyforms_form_type( $form_slug ) !== 'registration' ? 'user' : 'post', $location_id );
			}

			$result = ( $del1 && $del2 && $del3 );
		}
