...
class BuddyFormsGeoMyWpLocateEntries {
	...
	public function get_address_from_item( $item ) {
		...
		if ( empty( $this->args['address_fields'] ) || 'address' === $this->args['address_fields'] ) {
			$address = self::get_address_component_from_item( $item );
		} else {
			$this->args['address_fields'] = ! is_array( $this->args['address_fields'] ) ? explode( ',', $this->args['address_fields'] ) : $this->args['address_fields'];

			$address = self::get_address_component_from_item( $item, $this->args['address_fields'] );
		}
	...
	public static function get_address_component_from_item( $item, $component = array( 'formatted_address' ), $glue = ' ' ) {
		if ( is_array( $component ) ) {
			...
			if ( in_array( 'formatted_address', $component ) ) {
				...
				$address = implode( $glue, $address_array );
			}
